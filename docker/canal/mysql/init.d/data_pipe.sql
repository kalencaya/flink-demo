CREATE
database if NOT EXISTS `data_pipe` default character set utf8mb4 collate utf8mb4_unicode_ci;
use
`data_pipe`;
...
(
    ...
    `o_id`               bigint      NOT NULL COMMENT 'ERP 内部订单号，唯一',
    `shop_id`            bigint               DEFAULT NULL COMMENT '店铺编号',
    `co_id`              bigint               DEFAULT NULL COMMENT '公司编号',
    ...
    `seller_flag`        int                  DEFAULT NULL COMMENT '旗帜(1红旗，2黄旗，3绿旗，4蓝旗，5紫旗)）',
    `modified_begin`     datetime    NOT NULL COMMENT '修改起始时间',
    `modified_end`       datetime    NOT NULL COMMENT '修改结束时间',
    `page_index`         int         NOT NULL COMMENT '页数',
    `page_size`          int         NOT NULL COMMENT '条数',
...
) ENGINE=InnoDB COMMENT='聚水潭订单查询接口';
...
(
    ...
    `page_size`    int         NOT NULL DEFAULT '50' COMMENT '页数',
    `creator`      varchar(32) NOT NULL COMMENT '创建人',
    `modifier`     varchar(32) NOT NULL COMMENT '修改人',
    `is_deleted`   tinyint     NOT NULL DEFAULT '0' COMMENT '删除标识。1: 已删除, 0: 未删除',
    `gmt_create`   timestamp   NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
